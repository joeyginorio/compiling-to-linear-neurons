...
from mnist_cnn.mnist_one import CNN_One
...
class ModelD(nn.Module):
    def __init__(self):
        ...
        # 80k params
        self.check = CNN_One()
        checkpoint = torch.load("data/mnist_cnn_debug.pt", map_location="cpu")
        self.check.load_state_dict(checkpoint)
        self.check.eval()
        for p in self.check.parameters():
            p.requires_grad = False
        self.check_lin = nn.Sequential(
            self.check,
            nn.Linear(2,8),
            nn.Linear(8,2)
        )
    ...
    def forward(self, x):
        ...
        then_val = self.then_branch(x)
        return b1 * if_val + b2 * then_val
    ...
    for batch_size in batch_sizes:
        for lr in learning_rates:   
            for lam in lams:
                ...
                for idx in idxs:
                    output = modelD(test_ds[idx][0].unsqueeze(0).to(device)).squeeze().tolist()
